...
我们的官方 XFCE rootfs 压缩包已预置 **XFCE 自动启动**配置：启用 Termux:X11 后启动容器，XFCE 会自动启动并出现在 Termux:X11 应用中，无需任何终端命令。从[Rootfs 仓库](./Documentation/zh-CN/Usage-Android-App.md#rootfs-repository)（搜索"XFCE"）下载，或直接从 [Droidspaces Rootfs Builder Releases](https://github.com/Droidspaces/Droidspaces-rootfs-builder/releases/latest) 下载。
